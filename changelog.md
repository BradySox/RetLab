...
* **[Mission Generation]** **Kneeboard coordinates west of Greenwich and south of the equator are correct.** pydcs printed 115°18'W as `-115°42'00"W`: a stray minus and the wrong minutes and seconds. Every coordinate on the Nevada and South Atlantic kneeboards was wrong, and any point west of Greenwich on Normandy/Channel. The briefing page's bullseye, the SEAD/DEAD target table, the strike page's target locations and the recon aimpoint table now print `115°18'00"W`. A seconds value that rounded to 60 (`42°07'60"N`) now carries to the next minute. Every other coordinate prints exactly as before. Takes effect on the next generated mission.
* **[UI]** **Settings, plugin options and the map say what the game does.** An audit of every settings page, the New Game wizard, the other windows, the map and the plugin pages corrected text describing removed or changed features: cruise-missile raids "respecting ROE zones", AI procurement "reading the strategic picture", the threat-intel page waiting for "a TARPS overflight", COIN effects draining a "mandate" that no longer exists. False instructions are fixed too: closing Air Wing Configuration discards your changes, it does not keep them. Plugin options now show their written explanations. The plugin pages are **Lua Plugins** and **Lua Plugin Options**; the CSAR settings sit together under **Difficulty & Realism → CSAR flights**; UI text is US English. The SITREP lists downed pilots still waiting as "Awaiting rescue" instead of "MIA". Nothing to configure.
* **[Campaign]** **UH-60L, OH-6, JAS 39, Frenchpack and the Spanish navy pack can be turned on again.** The New Game Mods page had no toggle for them, so their units were stripped from every new game even where the campaign preseeds them: the UH-60L in 13 campaigns, the OH-6 in Yankee Station and Velvet Thunder. The toggles are back and follow the campaign's preseed; untick any mod you do not have installed. NEW game required.
* **[Campaign]** **CAS and Armed Recon engagement ranges default to upstream's 10 and 5 NM.** The wider 15 and 10 NM from June were never part of the planner re-convergence; the one-click **RetLab suite** on the RetLab Features page now sets them. Existing saves keep their values, and so does a saved Default.zip.
* **[Campaign]** **A campaign that preseeds a renamed setting keeps its value.** The New Game wizard skipped the migration a save load runs, so a campaign preseeding an old key (for example `eplrs_enabled`) got the new setting's default.
* **[Campaign]** **Pilot awards are removed.** The logbook no longer grants awards, and the SITREP no longer lists an award line for each pilot who earned one. On a first turn that was nearly every pilot who flew, AI included. Ranks, the logbook numbers and the lifetime pilot profiles are unchanged, and `resources/pilot_career.yaml` now carries rank ladders only. A save made before this loads without its awards.
...
## Fixes
* **[Mission Generation]** **F-15E: player-added JDAM targets no longer overwrite planned ones.** A Strike Eagle's planned targets are written as smart-weapon CC missions, 8 to a set. Targets you add by hand always started at set 2, so on a target with more than 8 aimpoints two missions shared one number. They now start at the next empty set. Nothing is written past the jet's 40-mission limit. The Strike kneeboard page labels each target `CC 1/1`, as the jet's Smart Weapons page shows it, instead of `DTC M1.1`. Takes effect on the next generated mission.
* **[Mission Generation]** **F-4E: the flight frequency is on COMM, not AUX.** AUX is receive-only, so a player Phantom could hear the flight but not talk to it. Channel 1 on COMM is now the flight frequency; the other presets follow from channel 2.
* **[Mission Generation]** **F-4E: the Pave Spike pod gets the flight's laser code.** The bombs were set to the allocated code while the pod stayed on 1688, so a Phantom lasing for its own bombs missed unless the WSO re-dialled the pod. The pod now starts on the kneeboard code.
* **[Mission Generation]** **P-51D and P-47D: Buttons B and C hold the tower's VHF frequency.** The SCR-522 tunes 100-156 MHz and was given the tower's UHF frequency (250 MHz on Normandy). A field with no VHF tower leaves the button empty. The kneeboard's airfield table shows the frequency on the button.
* **[Mission Generation]** **F-14A Early and F-14A-95-GR get the ST and IP points.** Only the F-14B and F-14A-135-GR had them. The same two variants and the F-14B(U) now spawn hot at a field with no ground power, as the others already did: the Tomcat has no APU.
* **[Mission Generation]** **Viper: the kneeboard no longer numbers steerpoints the jet does not have.** The data cartridge writes 20 route steerpoints; a longer route's kneeboard went on to "21 LANDING" while the jet's 21 was a support orbit. Rows past 20 now read "-".
* **[Mission Generation]** **Apache: data-cartridge target points sit on the ground.** They were written at sea level, so the TADS cued below a site on high ground. They now take the nearest airfield's elevation.
* **[Mission Generation]** **UH-1H: the kneeboard calls the presets UHF, not COM3.** Transmit selector 3 is the VHF radio; the presets are on the UHF set, selector 2.
* **[Mission Generation]** **F-15E: the flight frequency picker offers only UHF.** The flight frequency goes on radio 1, the UHF-only ARC-164, but the aircraft data named the radios the other way round, so the picker offered VHF and FM frequencies radio 1 cannot tune. Automatic frequencies were already UHF and do not change.
* **[Campaign]** **A campaign that placed IADS buildings but never set `advanced_iads` now runs in range mode.** The flag only chooses the network mode; the command centres, comms towers and power stations the author placed are what range mode wires to. Three shipped campaigns placed them without the flag — Desert Sabre, Allied Sword and Retake the Falklands — so their SAMs stood beside power stations that protected nothing. The flag is now on whenever the mission file places any of the three buildings, whatever the yaml says; the New Game wizard's Advanced IADS box is the per-game opt-out, and a campaign with no buildings is unchanged. NEW game required.
...
* **[Mission Generation]** **Anti-ship packages began their run 30 nm inside the Harpoon's reach.** The AGM-84A and AGM-84D carried the real Block 1C figure, 67 nm, and the ingress point is placed at the weapon's range. DCS flies the missile to 180 km, 97 nm, and that is the number the yaml carries now. Takes effect on the next planned turn.
* **[Mission Generation]** **Mavericks now declare their range, 13 nm (6 nm for the laser E).** No AGM-65 carried one, so a Maverick flight was left out of its package's ingress number: fragged beside a Harpoon flight, it was dragged out to the Harpoon's 97 nm run-in. A package's run-in is set by its shortest shooter, so a mixed package now starts at the doctrine's bound. A package of Mavericks alone is unchanged: the ingress only ever widens.
* **[Mission Generation]** **The naval weapons-release stagger no longer hands one side a head start.** Groups were released in control-point order, which lists one coalition's fleets before the other's, so one side's last group was released up to 13 minutes after the other's first. The order now alternates sides. Only with *Stagger naval weapons release* on.
