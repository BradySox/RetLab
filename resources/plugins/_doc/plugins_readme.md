...
- `specificOptions` : a list of specific plugin options
  ...
  - `enabledWhen` : *optional*. Greys this option out when another option of the **same plugin** is off, so a knob that the script cannot reach reads as inert instead of live. Write `"mnemonic"` to mean "enabled while that option is true", or `["mnemonic", value]` to match an explicit value. The master must be a boolean option of this plugin, and it must not itself declare `enabledWhen` — one level only. An unknown or self-referential mnemonic is a load-time error, not a silently permanently-greyed option.
    - Greying is display only: the option keeps its value and is still passed to the configuration script. Declare it only where the Lua genuinely ignores the option — check the script, do not infer it from the names. `airboss.rescueHeloDistance` reads like a rescue-helo knob and is not; it feeds the carrier controlled area whether or not the rescue helo is on.
