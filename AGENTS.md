...
62. **Squadron-sequenced modexes** — per-squadron blocks numbered in sequence for Hornets; the Tomcat paints its number into the livery, so its squadrons fly a CAG bird and line jets instead. The Payload tab can pin a flight's number (wingmen follow); no other package reuses it.
...
- **Open (awaiting review) — re-verified with `gh` 2026-08-30, 9 PRs, plus #966 opened 2026-09-12:** #966 (draft) · #950 (draft) · #925 (draft) · #920 · #896 · #884 · #881 · #874 · #872 · #792 · #788. **#886 is MERGED** (2026-08-28) and #955 is closed — see the 08-30 refresh above. Everything else the ledger once listed here is closed; see **Closed on review** below.
  ...
  - [#874](https://github.com/dcs-retribution/dcs-retribution/pull/874) curated carrier comms (**draft**) — §65 verbatim (per-hull boat cards feeding the DCS-rendered CV Operations Data page: hull-number TACAN + boat ident with `alloc_near` nearest-neighbor degrade, hull-keyed ICLS via a shared `IclsAllocator`, 336-band Link 4, stable persisted ATC, flagship named by hull name). NO fork couplings; the port adds only the Pretense allocator-type adaptation (behavior untouched). On dev @ `ef576acc`; pytest/Black/mypy green — opened 2026-07-16. Fork side = [RetLab#611](https://github.com/BradySox/RetLab/pull/611). See upstreaming-inventory item 19. **Widened 2026-09-23 (DM call) to a Navy pass**: upstream `dev` @ `49e8067f` merged in (`568e5930`) and §62's board numbers added as a second, isolated commit (`afa5d01b`: `ModexAllocator`, `LiveryAllocator`, the F-14B(U) High/Low Vis pairs collapsed to one livery-set preset per squadron; fork-only QRA/§61 hooks and `TARPS` left out), then the pinned board number and the X00-only CAG livery rule (`0e1ccf92`). Head `8b81b516`; Black / mypy / 524 tests green. **The title and body still describe comms only** — the replacement is in [docs/dev/retlab-pr874-body-refresh.md](docs/dev/retlab-pr874-body-refresh.md); pasting it is a description edit, allowed under the freeze.
