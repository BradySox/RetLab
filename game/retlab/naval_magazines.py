"""Cross-turn naval magazines (§81) — the fleet's anti-ship missiles are finite.
...
* **N2, the magazine** (``naval_magazines``) — each naval group carries a
  ...
  re-generating a mission is free (the §54/§63 lesson).

**No double count with §63.** The land-attack cruise missiles §63 meters
(Tomahawk, the 3M14 Kalibr) are absent from :data:`ASHM_WEAPON_PATTERNS` by
construction — the two magazines meter disjoint weapon sets, so a §63 raid
...
``"<id> | <name>"`` string §63's magazines use — the ``TheaterGroup`` lives in
...
#: ships at first sight. Curated by hand (the §41/§63 pattern — neither DCS nor
...
#: Substring patterns (case-insensitive, plain — never Lua patterns, the §70
...
#:
#: **Land-attack families are excluded on purpose** so §63's cruise-missile
...
#: case-inconsistent, so membership is checked lowercased (the §63 helper).
