...
#: names carry ``-`` and ``(`` which a pattern match would read as magic; the §70
#: lesson), so ``GBU-31`` catches every ``GBU-31(V)*/B`` variant at once.
#:
...
#: not a feature. Ship-launched land-attack cruise missiles (§63/§81) are
    ...
    for cp in game.theater.controlpoints:
        for tgo in cp.ground_objects:
            ...
            try:
                ...
                pass  # a TGO/test double with no fog leaf is simply visible
            briefed.append(site)
    return briefed
