...
| 2026-09-23 | not re-checked | **Nothing taken; a gap found on test 39 and closed fork-side.** A loss with no ejection placed its survivor near the package target (`fallback_position_for`), 23–42 km from the crash for nine blue survivors; turn 2 then fragged a CSAR 41 km off. The base script now records `crash_positions` on `S_EVENT_CRASH` and the survivor is placed there (else the last §91 track sample, else the target). Checklist B142, inventory item 41. |
