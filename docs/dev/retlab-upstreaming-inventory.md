...
| 41 | **A survivor with no ejection is placed at his package's target** — upstream #929's `fallback_position_for` scatters round the target for every loss DCS reported no ejection for, flown or simulated. Found on the fork's test 39, 2026-09-23: nine survivors 23–42 km from their jets, a CSAR fragged 41 km off. Fork fix: `crash_positions` on `S_EVENT_CRASH` in `dcs_retribution.lua`, used before the target scatter | 🕐 LAST-MILE | **A comment on #929, not a PR** — same shape as item 38. The §91 track fallback is fork-only and would be stripped | B142 ☐ |
| 42 | **Runway queue at busy fields** (§104: `game/ato/runwayqueue.py` + the one-line `estimate_ground_ops` hook; parking-start fixed-wing flights queue by planned takeoff at 45 s per jet, 8 min base kept; no setting, DM call 2026-09-23; 14 tests; calibrated on 494 flown AI groups, late > 2 min 25 % → 10 %) | 🟡 NEAR — unflown | Medium/High — the traffic half of upstream [#214](https://github.com/dcs-retribution/dcs-retribution/issues/214), raised in its thread and never modelled. Self-contained, no Lua. Upstream's reviewers asked for options on behaviour changes before (#794), so expect a setting request | B145 ☐ |
...
| ✔ [#561](https://github.com/dcs-retribution/dcs-retribution/issues/561) Frontline unit naming | §59 | The title is about naming; the body asks for Tiresias, which turns off AI for ground units far from opposing aircraft. §59 built that natively and was removed 2026-09-23 (DM call), so the fork no longer answers this issue. ⚠️ **Already claimed upstream** by draft [PR #568](https://github.com/dcs-retribution/dcs-retribution/pull/568) (cedriclmenard), which ports Tiresias itself |
...
| [#214](https://github.com/dcs-retribution/dcs-retribution/issues/214) Configurable startup times | queue items 29 and 42 (§104 answers the airport-traffic comment) | Known |
